...
- 设置页分区按模式过滤：简易模式隐藏 AES 加密高级管理、风险操作黑名单、CI 流水线、Cloudflare 部署 Hook、诊断日志导出等开发配置分区；保留基本信息、模式开关、Git Token 档案、同步定时、存储目录、AI 模型配置、AI 密钥管理、代理、图床分区。
- 设置页提供「简易普通用户模式」开关（Switch），开启/关闭即切换模式并实时重建界面。
- 分区可见性复用 `ModeVisibilityFilter` 语义（`settings` 分组入口定义），确保双端一致。
