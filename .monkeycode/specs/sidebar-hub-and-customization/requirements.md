...
### Requirement 6: 全部博客管理快照缓存

**User Story:** AS 用户, I want 打开全部博客管理时秒开, so that 重复查看无需等待网络重新加载。

#### Acceptance Criteria

1. WHEN 用户首次打开"全部博客管理"，THEN 系统 SHALL 从远端加载全部仓库文章，并将结果作为快照缓存持久化。
2. WHEN 用户再次打开"全部博客管理"，THEN 系统 SHALL 先展示缓存快照，并仅在对应仓库有文章更新时重新拉取该仓库。
3. WHEN 系统判断仓库是否有更新，THEN 系统 SHALL 以仓库最近提交/文件变动时间为依据，与缓存时记录的基线比对。
4. WHEN 所有仓库均无更新，THEN 系统 SHALL 直接使用缓存快照渲染，不发起网络请求。
5. WHEN 某个仓库有更新，THEN 系统 SHALL 只重新加载该仓库的部分文章，其余仓库沿用缓存。
6. WHEN 用户显式点击"刷新"按钮，THEN 系统 SHALL 强制全部仓库重新加载并更新缓存基线。
7. WHEN 缓存文件缺失或读取失败，THEN 系统 SHALL 回退为全量重新加载，不阻断页面展示。