...
The search script lives inside this skill's own directory, not the project directory. Always invoke it by its full path — do not assume a particular working directory. Under opencode, the skill's base directory is this skill's folder (`.opencode/skills/ui-ux-pro-max/`), so call the script relative to it:
