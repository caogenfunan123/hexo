    ...
    Navigator.of(context).push(MaterialPageRoute(
      builder: (_) => AiArticleChatScreen(
        ...
        initialMessage: '你是一键建站助手。用户想要创建一个新的静态博客站点。\n\n'
            ...
            '用户确认全部信息后，调用 create_site 工具完成建站（可传 root_domain 参数）。'
            ...
            '若自检失败，根据失败项用 file_write 修改仓库内容（站点配置、主题、文章等）触发重新构建，'
            '再次 verify_site 复查直到站点正常。'
            '若 create_site 失败，可改用分步工具断点续跑自愈：先调用 create_repo 建仓库（可传 root_domain），'
