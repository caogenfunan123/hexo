...
extension DesktopShellModeUiExt on DesktopShellState {
  Future<void> _refreshWallpaperBrightness() async {
    ...
    try {
      ...
      if (data != null) {
        ...
        final count = (bytesData.length / (4 * (step > 400 ? step ~/ 400 : 1)))
            .ceil();
  ...
  Widget _buildTopBar(LayoutController layout) {
    // 专注与写作画布共用极简标题栏（保存/预览/更多菜单）；工作台用完整标题栏
    if (layout.workMode != WorkMode.workspace) return _focusModeTitleBar();
  ...
  Widget _buildMainArea(LayoutController layout) {
    ...
    final mainContent = switch (layout.workMode) {
      ...
      WorkMode.source => _buildCanvasEditor(),
    ...
    return Row(
      children: [
        ...
        if (layout.rightDrawerOpen)
          RepaintBoundary(child: _buildRightDrawer(layout)),
      ],
    );
  }
  ...
  Widget _buildBottomBar(LayoutController layout) {
    ...
    return ListenableBuilder(
      ...
      builder: (context, child) {
        ...
        final readMin = charCount > 0
            ? (charCount / 400).ceil().clamp(1, 120)
            : 0;
          ...
          children: [
            ...
            PopupMenuButton<String>(
              ...
              itemBuilder: (_) => [
                ...
                PopupMenuItem(
                  value: 'ai',
                  child: _focusMoreItem(Icons.auto_awesome, 'AI 对话', isDark),
                ),
                PopupMenuItem(
                  value: 'publish',
                  child: _focusMoreItem(Icons.send_outlined, '一键发布', isDark),
                ),
                PopupMenuItem(
                  value: 'sync',
            ...
            const SizedBox(width: 4),
            Container(width: 1, height: 20, color: AppColor.border(context)),
  ...
  Widget _focusContextMenu(BuildContext context, EditableTextState state) {
    ...
    final chips = <Widget>[
      ...
      if (!hasSelection) ...[
        _miniToolbarChip(Icons.content_paste, '粘贴', () async {
  ...
  Widget _miniToolbarChip(IconData icon, String label, VoidCallback onTap) {
    return Tooltip(
      message: label,
      child: Padding(
  ...
  Widget _focusMoreItem(
    ...
    return Row(
      children: [
        Icon(icon, size: 16, color: AppColor.icon(context)),
  ...
  Widget _buildFocusEditor() {
    ...
    return Stack(
      children: [
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(height: 1, color: mutedColor.withOpacity(0.6)),
                  const SizedBox(height: 16),
                  // 正文编辑区（带当前行高亮）
                  Stack(
                    children: [
        ...
        // ── 浮层：右侧实时预览 / 右抽屉（与专注模式共用） ──
        ..._buildCanvasOverlays(mutedColor: mutedColor),
      ],
    );
  }

  /// 专注/画布模式共用浮层：右侧实时预览面板（极简标题栏「预览」打开）
  /// 与右抽屉（元数据/AI），两者互斥。
  List<Widget> _buildCanvasOverlays({required Color mutedColor}) {
    return [
        ...
        Positioned(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: AppColor.surfaceRaised(context),
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: AppColor.borderStrong(context)),
              boxShadow: [
                BoxShadow(
  ...
  /// 写作画布（原源码模式，阶段6改版）：
  /// 标题+正文一张连续画布，所见即所得渲染，支持自定义壁纸，对标手机端编辑页。
  /// 纯 Markdown 源码编辑保留在工作台分栏编辑器的「源码」模式。
  Widget _buildCanvasEditor() {
    final cs = Theme.of(context).colorScheme;

    // 背景层：自定义壁纸 / 纯黑 / 主题底色（对标手机端 _buildEditorBackground）。
    // 非自定义背景时跟随明暗主题，修正旧画布在暗色主题下白底白字的问题。
    final Widget bgLayer;
    if (_deskUseWallpaper) {
      bgLayer = Image.file(
        File(_deskEditorTheme.wallpaperPath),
    ...
    } else if (_deskCustomBg) {
      bgLayer = ColoredBox(color: _deskBgColor);
    } else {
      bgLayer = ColoredBox(color: AppColor.surfaceBase(context));
    }

    final textColor = _deskCustomBg ? _deskTextColor : null;
    final fallbackText = textColor ?? AppColor.textPrimary(context);
    ...
    return Stack(
      children: [
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  hintStyle: TextStyle(
                    fontSize: 24,
                    fontWeight: FontWeight.w700,
                    color: fallbackText.withOpacity(0.3),
                  ),
                  border: InputBorder.none,
                  enabledBorder: InputBorder.none,
                  focusedBorder: InputBorder.none,
        ...
        // ── 浮层：右侧实时预览 / 右抽屉（与专注模式共用） ──
        ..._buildCanvasOverlays(
          mutedColor: fallbackText.withOpacity(0.35),
        ),
      ],
    );
  }
      ...
      child: MouseRegion(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border(right: BorderSide(color: AppColor.border(context))),
          ),
          child: Center(
            child: Icon(
              Icons.chevron_right,
