...
class SiteIsolationService {
  ...
  Future<void> encryptSiteData(String siteId, String password) async {
    ...
    for (final entity in files) {
      ...
      try {
        ...
        // 加密成功后删除原文
        await file.delete();
      } catch (e) { print('SiteIsolation: save state failed: $e');
        // 加密失败，跳过此文件
      }
    }
  ...
  Future<void> decryptSiteData(String siteId, String password) async {
    ...
    for (final entity in files) {
      ...
      try {
        ...
        // 解密成功后删除加密文件
        await file.delete();
      } catch (e) { print('SiteIsolation: load state failed: $e');
        // 解密失败，跳过此文件
      }
    }
  ...
  Future<bool> verifySitePassword(String siteId, String password) async {
    ...
    for (final entity in files) {
      ...
      try {
        ...
        return true;
      } catch (e) { print('SiteIsolation: load index failed: $e');
        return false;
      }
    }
    return false;
  ...
  Future<void> _loadIndex() async {
    ...
    } catch (e) { print('SiteIsolation: save config failed: $e');
      // 索引文件损坏，从各站点目录恢复
      await _recoverIndex();
    }
  }
      ...
      if (entity is Directory) {
        ...
        if (await configFile.exists()) {
          try {
            ...
            _siteConfigs[config.id] = config;
          } catch (e) { print('SiteIsolation: delete site failed: $e');
            // 跳过损坏的配置
          }
        }
      }
