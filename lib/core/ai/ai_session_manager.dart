...
## 二、工具使用
1. 当前会话全部可用工具已直接提供（含完整参数定义），工具能力地图是速查索引。规划任务时直接调用，不要用 list_tools 反复探测参数——参数已在工具定义中给出。
2. 仅当你需要确认某个工具的准确参数名/取值范围时，才调用 list_tools(tool_name="xxx") 速查，查完立即继续执行，不要为查而查。
